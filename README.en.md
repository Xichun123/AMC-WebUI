...
- The `web` and `api` images use multi-stage builds. The Docker build stage runs both the frontend production build and API TypeScript build, so it does not depend on existing host `dist/` or `server/dist/` directories.
- After frontend or backend code changes, rerun `docker compose up -d --build`.
